...
export async function getSourceDirectory(): Promise<SourceGroupItem[]> {
...
export type SourceGroupDetail = {
  ...
  starredCount: number;
  lastUpdate: string;
  channels: SourceChannelStat[];
  dailySeries: SourceDailySeries[];
  categories: SourceCategoryCount[];
  importanceDistribution: Array<{ level: string; count: number }>;
};

export async function getSourceGroupDetail(departmentName: string): Promise<SourceGroupDetail | null> {
  const pool = getPgPool();
...
export async function getSourceTopItems(departmentName: string, limit: number = 20): Promise<SourceTopItem[]> {
  const pool = getPgPool();
