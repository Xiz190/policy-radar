...
export function useItemList({ queryKey, pageSize = 15, buildParams, setSourcesTree }: UsePolicyListOptions) {
  ...
  async function loadItemsRaw(currentPage: number) {
    ...
    try {
      ...
      const list: ContentItem[] = (json.items || []).map((raw) => {
        ...
        return {
          ...
          categories: Array.isArray(r.categories) ? (r.categories as ContentItem["categories"]) : [],
