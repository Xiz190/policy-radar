...
import { getLatestMctDecisionNews } from "@/lib/live-mct-jyjd";
import {
...
export default async function MctJyjdLivePage() {
  const result = await getLatestMctDecisionNews(10);
      ...
      <div className="mx-auto flex w-full max-w-6xl flex-col gap-8 px-6 py-8 lg:px-10">
          ...
          <div className="grid gap-8 lg:grid-cols-[1.4fr_1fr]">
            <div className="space-y-4">
              ...
              <h1 className="text-4xl font-semibold tracking-tight">
                文化和旅游部政府信息公开 · 体裁分类 / 决议决定
              </h1>
              <p className="max-w-3xl text-base leading-7 text-slate-300">
                这页实时从 `体裁分类 / 决议决定` 列表抓取内容，但只保留 `2026-05-05` 之后的记录，
                并在过滤后只展示最新 {MONITOR_MAX_ITEMS} 条，同时解析详情页发布时间、发布机构、文号、分类和正文前几段。
              </p>
            </div>

            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-4">
                <div className="text-sm text-slate-300">栏目</div>
                <div className="mt-1 text-lg font-semibold">决议决定</div>
              </div>
              <div className="rounded-3xl border border-white/10 bg-white/5 px-4 py-4">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-2">
            <div className="rounded-2xl bg-slate-50 p-4">
              ...
              <div className="mt-2 text-sm font-medium text-slate-900">体裁分类 / 决议决定</div>
            </div>
            <div className="rounded-2xl bg-slate-50 p-4 text-sm leading-6 text-slate-600">
              当前规则：
              <div>1. 只保留 {MONITOR_START_DATE} 之后的内容</div>
              <div>2. 过滤后只展示最新 {MONITOR_MAX_ITEMS} 条</div>
              <div>3. 进入详情页解析发布时间、发布机构、文号与分类</div>
              <div>4. 默认按站内正式公开内容处理</div>
            </div>
          </div>
        </section>
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
          <div className="flex items-center justify-between gap-4">
            <div>
              <h2 className="text-2xl font-semibold">最新决议决定</h2>
              <p className="mt-1 text-sm text-slate-500">
                {visibleItems.length > 0
                  ? `当前按列表页顺序展示 ${MONITOR_START_DATE} 之后的最新条目，默认优先保留详情页跳转。`
                  : MONITOR_EMPTY_TEXT}
              </p>
            </div>
          </div>
            ...
            {visibleItems.length > 0 ? visibleItems.map((item, index) => (
              <div key={item.url} className="rounded-3xl border border-slate-200 p-5">
                ...
                <div className="mt-4 grid gap-3 md:grid-cols-2">
                  <div className="rounded-2xl bg-slate-50 p-4">
                    <div className="text-sm text-slate-500">详情页发布时间</div>
                    <div className="mt-2 text-sm font-medium text-slate-900">
                      {item.detailPublishedAt ?? "暂未解析到"}
                    </div>
                    <div className="mt-1 text-sm text-slate-600">
                      {item.classificationText ? `分类：${item.classificationText}` : "分类待识别"}
                    </div>
                  </div>

                  <div className="rounded-2xl bg-slate-50 p-4">
                    <div className="text-sm text-slate-500">站内发布判断</div>
                    <div className="mt-2 text-sm font-medium text-slate-900">{item.originJudgment}</div>
                    <div className="mt-1 text-sm leading-6 text-slate-600">{item.originReason}</div>
                  </div>
