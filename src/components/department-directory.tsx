...
export function DepartmentDirectoryBrowser({
  departments,
  summary,
}: {
    ...
    <div className="space-y-4">
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {filtered.map((d) => (
            <DepartmentCard key={d.departmentName} d={d} today={today} />
          ))}
        </div>
      )}
    </div>
...
function DepartmentCard({ d, today }: { d: DepartmentCardData; today: string }) {
