...
export type DepartmentDailySeries = { date: string; count: number; urgent: number; highlight: number };
export type DepartmentCategoryCount = { category: string; label: string; count: number };
...
export type DepartmentDetailView = {
  ...
  firstSeenAt: string | null;
  latestPublishedAt: string | null;
  channelStats: DepartmentChannelStat[];
...
// ====== 30 日趋势折线图（纯 SVG） ======
function DailyLineChart({ series }: { series: DepartmentDailySeries[] }) {
  if (series.length === 0) {
...
// ====== 分类条形图 ======
function CategoryBarChart({ categories }: { categories: DepartmentCategoryCount[] }) {
...
// ====== Top 内容行 ======
function TopItemRow({ item, departmentName }: { item: DepartmentTopItem; departmentName: string }) {
