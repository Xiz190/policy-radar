...
import {
  Landmark, Link, Paperclip, Tag,
} from "lucide-react";

export type RelatedPolicyItem = {
  sourceId: string;
  url: string;
  title: string;
...
export type RelatedPoliciesData = {
  sameTopic: RelatedPolicyItem[];
  sameDept: RelatedPolicyItem[];
  cited: RelatedPolicyItem[];
};

type RelatedPoliciesProps = {
  data: RelatedPoliciesData;
};

const TAB_CONFIG = [
  { key: "sameTopic", label: "同领域", icon: Tag, description: "标签高度重合" },
  { key: "sameDept", label: "同部委", icon: Landmark, description: "同一发布单位的政策" },
...
export function RelatedPolicies({ data }: RelatedPoliciesProps) {
    ...
    <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-base font-semibold text-slate-900">相关政策</h2>
