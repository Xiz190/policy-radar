...
const TEST_CASE = { name: 'items-grouped', path: '/api/monitor/items?view=grouped', method: 'GET' };
...
async function makeRequest(testCase) {
  ...
  return new Promise((resolve) => {
    ...
    req.on('error', (err) => {
      const duration = Date.now() - start;
      resolve({ success: false, statusCode: 0, duration, error: err.message });
    });

    req.on('timeout', () => {
      req.destroy();
      const duration = Date.now() - start;
      resolve({ success: false, statusCode: 408, duration, error: 'timeout' });
    });
...
async function runTest() {
  console.log('\n========================================');
  console.log('  items-grouped 接口专项压力测试');
  console.log('  (Promise.all 并行优化后)');
  console.log('========================================\n');

  console.log(`测试接口: ${TEST_CASE.name}`);
  console.log(`请求路径: ${TEST_CASE.path}\n`);

  let allResults = [];

  for (const stage of CONCURRENCY_STAGES) {
    console.log(`[阶段] ${stage.label} (并发: ${stage.concurrency}, 时长: ${stage.duration}s)`);
    const result = await runStage(stage.concurrency, stage.duration, TEST_CASE);
    ...
    console.log(`  响应时间(ms): avg=${result.avgDuration} p50=${result.p50} p95=${result.p95} p99=${result.p99}`);
  ...
  console.log('');
  console.log('  优化前 (串行):');
  console.log('    低负载 RPS: 22.76');
  console.log('    峰值负载成功率: 0%');
  console.log('    峰值响应时间: ~10000ms');
  console.log('');
  console.log('  优化后 (并行):');
